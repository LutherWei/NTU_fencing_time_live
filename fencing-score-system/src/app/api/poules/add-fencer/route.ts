...
export async function POST(request: Request) {
  try {
    ...
    if (category.status === 'elimination' || category.status === 'finished') {
      return NextResponse.json({ success: false, error: '分組賽階段已結束，無法再新增參賽者' }, { status: 400 })
    }

    const isTeam = category.competitionType === 'TEAM'

    // find poule with min fencers/teams
    let targetPoule = category.poules[0]
    for (const p of category.poules) {
      const pCount = isTeam ? p.teams.length : p.fencers.length;
      const targetCount = isTeam ? targetPoule.teams.length : targetPoule.fencers.length;
      if (pCount < targetCount) {
        targetPoule = p
      }
    }
    ...
    if (isTeam) {
      ...
      // Create matches against all existing teams in targetPoule
      const createMatchesPromises = targetPoule.teams.map(t => {
        const isT1First = t.id < newTeam.id
        return prisma.pouleMatch.create({
          data: {
            pouleId: targetPoule.id,
            team1Id: isT1First ? t.id : newTeam.id,
            team2Id: isT1First ? newTeam.id : t.id
          }
        })
      })
    ...
    return NextResponse.json({ success: false, error: '新增參賽者失敗' }, { status: 500 })
  }
}
